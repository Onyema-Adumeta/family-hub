...
exports.default = awardBadges;
exports.startStreakCron = startStreakCron;
const node_cron_1 = __importDefault(require("node-cron"));
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
async function awardBadges(memberId) {
    const member = await prisma.member.findUnique({ where: { id: memberId } });
    if (!member)
        return;
    const badges = new Set(member.badges || []);
    const newBadges = [];
    if (member.totalChoresDone >= 1 && !badges.has('first_chore')) {
        badges.add('first_chore');
        newBadges.push('first_chore');
    }
    if ((member.streakDays ?? 0) >= 7 && !badges.has('streak_7')) {
        badges.add('streak_7');
        newBadges.push('streak_7');
    }
    if ((member.streakDays ?? 0) >= 30 && !badges.has('streak_30')) {
        badges.add('streak_30');
        newBadges.push('streak_30');
    }
    if (member.stars >= 50 && !badges.has('star_50')) {
        badges.add('star_50');
        newBadges.push('star_50');
    }
    if (member.stars >= 100 && !badges.has('star_100')) {
        badges.add('star_100');
