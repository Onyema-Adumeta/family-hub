...
export default function SchedulePage() {
  ...
  function handleDayClick(dateStr: string) {
    ...
    setShowAdd(false);
    if (isMobile) setShowPanel(true); // on mobile, show right panel after tap
  }
  ...
  // ── Right Panel content (shared between mobile modal and desktop sidebar) ──
  const RightPanel = () => (
      ...
      <div className="card">
        <div style={{ fontWeight:900, fontSize:13, marginBottom:10, color:'var(--text-secondary)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button
            ...
            style={{ fontSize:11, fontWeight:800, padding:'4px 10px', borderRadius:20, background:'var(--primary)', color:'#fff', border:'none', cursor:'pointer' }}
          >+ Add</button>
        </div>
        ...
        {selectedEvents.length === 0 ? (
          ...
          return (
            <div key={ev.id} style={{
              ...
              {/* Delete — visible to parents and the creator */}
              <button
                onClick={() => { if (window.confirm('Delete this event?')) deleteEvent.mutate(ev.id); }}
                style={{
                  background:'rgba(248,113,113,0.1)', border:'1px solid rgba(248,113,113,0.25)',
                  borderRadius:8, cursor:'pointer', color:'#F87171',
                  fontSize:13, padding:'6px 8px', flexShrink:0,
        ...
        {upcomingEvents.length === 0 ? (
          ...
          return (
            <div key={ev.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 0', borderBottom:'1px solid var(--border)' }}>
              ...
              <button
                onClick={() => { if (window.confirm('Delete this event?')) deleteEvent.mutate(ev.id); }}
                style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', fontSize:13, padding:4, flexShrink:0 }}
              >🗑️</button>
            </div>
          );
        })}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    padding:'4px', borderRadius:10, cursor:'pointer',
                ...
                >
                  ...
                  {dayEvents.slice(0, isMobile ? 1 : 2).map((ev: any) => (
                    ...
                    }}>{ev.emoji} {isMobile ? '' : ev.title}</div>
                  ))}
