...
export default function SchedulePage() {
  ...
  function deleteEv(id: string) {
    if (window.confirm('Delete this event?')) deleteEvent.mutate(id);
  }

  // ── Panel JSX (inlined, not a component) ──────────────────────
  const panelJsx = (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      <div className="card">
        <div style={{ fontWeight:900, fontSize:13, marginBottom:10, color:'var(--text-secondary)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <span>{selectedDate === todayStr ? '📌 Today' : new Date(selectedDate+'T12:00:00').toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'})}</span>
          <button onClick={() => setShowAdd(true)} style={{ fontSize:11, fontWeight:800, padding:'4px 10px', borderRadius:20, background:'var(--primary)', color:'#fff', border:'none', cursor:'pointer' }}>+ Add</button>
        </div>
        {selectedEvents.length === 0 ? (
          <div style={{ fontSize:12, color:'var(--text-muted)', fontWeight:700, padding:'8px 0', textAlign:'center' }}>No events — tap + Add</div>
          ...
          return (
            <div key={ev.id} style={{ display:'flex', alignItems:'flex-start', gap:8, padding:'10px 0', borderBottom:'1px solid var(--border)' }}>
              ...
              <button onClick={() => deleteEv(ev.id)} style={{ background:'rgba(248,113,113,0.1)', border:'1px solid rgba(248,113,113,0.25)', borderRadius:8, cursor:'pointer', color:'#F87171', fontSize:13, padding:'6px 8px', flexShrink:0 }}>🗑️</button>
            </div>
          );
        })}
      </div>
      ...
      <div className="card">
        ...
        {upcomingEvents.length === 0 ? (
          ...
          const dateLabel = d.toISOString().slice(0,10) === todayStr ? 'Today' : d.toLocaleDateString('en-US',{month:'short',day:'numeric'});
          return (
            <div key={ev.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 0', borderBottom:'1px solid var(--border)' }}>
              ...
              <button onClick={() => deleteEv(ev.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', fontSize:13, padding:4 }}>🗑️</button>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div style={{ width:'100%', maxWidth:900, paddingBottom:90 }}>
            ...
            {Array.from({ length: daysInMonth }, (_,i) => i+1).map(day => {
              ...
              return (
                <div key={day} onClick={() => handleDayClick(dateStr)} style={{
                  ...
                  {dayEvents.length > (isMobile?1:2) && <div style={{ fontSize:9, color:'var(--text-muted)', fontWeight:700 }}>+{dayEvents.length-(isMobile?1:2)}</div>}
                </div>
              );
            })}
          </div>
      ...
      {isMobile && showPanel && (
        <>
          <div onClick={() => setShowPanel(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:200, backdropFilter:'blur(2px)' }} />
