...
function StatCard({ icon, label, value, gradient }: { icon: string; label: string; value: string | number; gradient: string }) {
  return (
    <div style={{ padding: '14px 16px', borderRadius: 'var(--radius)', background: gradient, border: '1.5px solid rgba(255,255,255,0.08)' }}>
      <div style={{ fontSize: 22, marginBottom: 4 }}>{icon}</div>
      <div style={{ fontSize: 26, fontWeight: 900, lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.6)', marginTop: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</div>
    </div>
  );
}
...
function SectionTitle({ icon, title, action, onAction }: { icon: string; title: string; action?: string; onAction?: () => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
      <h2 style={{ fontSize: 15, fontWeight: 900, display: 'flex', alignItems: 'center', gap: 6 }}>
        <span>{icon}</span> {title}
      </h2>
      {action && (
        <button onClick={onAction} style={{ fontSize: 12, fontWeight: 700, color: 'var(--primary)', background: 'none', border: 'none', cursor: 'pointer', opacity: 0.85 }}>
          {action} →
        </button>
      )}
    </div>
...
function Empty({ icon, text }: { icon: string; text: string }) {
  return (
    <div style={{ textAlign: 'center', padding: '18px 0', color: 'var(--text-muted)', fontSize: 13, fontWeight: 600 }}>
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ width: '100%', maxWidth: 1100 }} className="animate-fadeIn">
      ...
      {/* AI tip */}
      <div style={{ marginBottom: 20, padding: '12px 16px', borderRadius: 'var(--radius)', background: 'linear-gradient(135deg,rgba(124,111,247,0.15),rgba(167,139,250,0.08))', border: '1.5px solid rgba(124,111,247,0.25)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <span style={{ fontSize: 20 }}>🤖</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 10, fontWeight: 900, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 2 }}>AI Insight</div>
          <div style={{ fontSize: 13, fontWeight: 600 }}>{tip}</div>
        </div>
        <button onClick={() => navigate('/chat')} className="btn btn-primary" style={{ fontSize: 12, padding: '7px 14px', flexShrink: 0 }}>
          Ask AI
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 10, marginBottom: 20 }}>
        <StatCard icon="⏱️" label="Pending"     value={pending.length}     gradient="linear-gradient(135deg,rgba(251,191,36,0.18),rgba(245,158,11,0.06))" />
        <StatCard icon="🔥" label="In progress" value={inProg.length}      gradient="linear-gradient(135deg,rgba(251,146,60,0.18),rgba(244,114,182,0.06))" />
        <StatCard icon="✅" label="Done today"  value={doneToday.length}   gradient="linear-gradient(135deg,rgba(74,222,128,0.18),rgba(16,185,129,0.06))" />
        <StatCard icon="📅" label="Events"      value={todayEvents.length} gradient="linear-gradient(135deg,rgba(56,189,248,0.18),rgba(99,102,241,0.06))" />
        <StatCard icon="⭐" label="Your stars"  value={member?.stars || 0} gradient="linear-gradient(135deg,rgba(124,111,247,0.18),rgba(167,139,250,0.06))" />
      </div>
                  ...
                  return (
                    <div key={c.id} onClick={() => navigate('/chores')} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, background: 'var(--bg-secondary)', cursor: 'pointer', border: '1.5px solid var(--border)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{a?.name || 'Anyone'} · ⭐ {c.stars}</div>
                      </div>
                      <span style={{ fontSize: 10, fontWeight: 800, padding: '3px 8px', borderRadius: 20, background: c.status === 'in_progress' ? 'rgba(251,191,36,0.15)' : 'var(--bg-tertiary)', color: c.status === 'in_progress' ? '#FBBF24' : 'var(--text-muted)' }}>
          ...
          <div className="card card-peach animate-fadeIn">
            ...
            {tonightMeal
              ? <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <span style={{ fontSize: 38 }}>{tonightMeal.emoji || '🍴'}</span>
                  <div style={{ fontWeight: 900, fontSize: 16 }}>{tonightMeal.name}</div>
                </div>
        ...
        <div className="card animate-fadeIn">
          ...
          {leaderboard.length === 0
            ...
            : leaderboard.map((m: any, i: number) => {
                ...
                return (
                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 10, background: i === 0 ? 'rgba(251,191,36,0.08)' : 'var(--bg-secondary)', border: `1.5px solid ${i === 0 ? 'rgba(251,191,36,0.2)' : 'var(--border)'}`, marginBottom: 6 }}>
            ...
            : recentMsgs.map((msg: any) => {
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 9, marginBottom: 10 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 11, fontWeight: 800, color: sender?.color || 'var(--primary)', marginBottom: 2 }}>{sender?.name || 'Family'}</div>
                      <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>{(msg.content || '').slice(0, 80)}</div>
                    </div>
                  </div>
                );
              })
        ...
        {activity.length > 0 && (
          <div className="card animate-fadeIn" style={{ gridColumn: '1 / -1' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {activity.map((a, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 20, background: 'var(--bg-secondary)', border: `1.5px solid ${a.color}33`, fontSize: 13, fontWeight: 600 }}>
