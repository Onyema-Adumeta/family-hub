...
function Leaderboard({ session, highlight, showReview, onToggleReview }: { session: TriviaSession; highlight?: string; showReview: boolean; onToggleReview: () => void; }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {myScore && (
        <div style={{ padding: '16px', borderRadius: 16, marginBottom: 4, textAlign: 'center', background: myScore.correct >= session.questions.length * 0.7 ? 'rgba(74,222,128,0.1)' : 'rgba(99,102,241,0.1)', border: `1.5px solid ${myScore.correct >= session.questions.length * 0.7 ? 'rgba(74,222,128,0.3)' : 'rgba(99,102,241,0.3)'}` }}>
          <div style={{ fontSize: 32, marginBottom: 6 }}>
            {myScore.correct === session.questions.length ? '🏆' : myScore.correct >= session.questions.length * 0.7 ? '🎉' : myScore.correct >= session.questions.length * 0.4 ? '👍' : '💪'}
          </div>
          <div style={{ fontSize: 28, fontWeight: 900, color: 'var(--text)' }}>{myScore.correct}/{session.questions.length}</div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 600, marginTop: 4 }}>
            {myScore.correct === session.questions.length ? 'Perfect score! Amazing!' : myScore.correct >= session.questions.length * 0.7 ? 'Great job!' : myScore.correct >= session.questions.length * 0.4 ? 'Good effort!' : 'Keep practicing!'}
          </div>
        </div>
      )}
      {ranked.length === 0 && <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 20, fontSize: 13 }}>No answers yet</div>}
      {ranked.map((s, i) => (
        <div key={s.member.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 14, background: s.member.id === highlight ? 'rgba(124,111,247,0.15)' : 'rgba(255,255,255,0.04)', border: `1.5px solid ${s.member.id === highlight ? 'rgba(124,111,247,0.4)' : 'rgba(255,255,255,0.08)'}` }}>
      ...
      {session.status === 'finished' && (
        <button onClick={onToggleReview} style={{ marginTop: 8, padding: '10px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', background: showReview ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)', border: `1.5px solid ${showReview ? 'var(--primary)' : 'rgba(255,255,255,0.1)'}`, color: showReview ? 'var(--primary)' : 'var(--text-muted)' }}>
              ...
              <div key={q.id} style={{ padding: '14px', borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {q.options.map(opt => {
                    ...
                    return (
                      <div key={opt} style={{ padding: '6px 10px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: isCorrect ? 'rgba(74,222,128,0.1)' : isMine && !isCorrect ? 'rgba(248,113,113,0.1)' : 'transparent', border: `1px solid ${isCorrect ? 'rgba(74,222,128,0.3)' : isMine && !isCorrect ? 'rgba(248,113,113,0.3)' : 'transparent'}`, color: isCorrect ? '#4ADE80' : isMine && !isCorrect ? '#F87171' : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
                        {opt.replace(/^[A-D]\) /, '')}
...
export default function TriviaPage() {
  ...
  useSocket((msg) => {
    if (msg.type === 'trivia:started') { setSession(msg.session); setCurrentQ(0); setSelected({}); setRevealed({}); setView('play'); setShowReview(false); }
  ...
  useEffect(() => {
    ...
    if (!q) return;
    if (session.answers.some(a => a.questionId === q.id && a.memberId === member?.id)) return;
    setTimeLeft(QUESTION_TIME);
    timerRef.current = setInterval(() => {
      setTimeLeft(t => { if (t <= 1) { clearInterval(timerRef.current); setRevealed(prev => ({ ...prev, [q.id]: true })); return 0; } return t - 1; });
    }, 1000);
  ...
  async function handleGenerate() {
    ...
    } catch (e: any) { alert(e.response?.data?.error || 'Failed to generate trivia'); }
  ...
  if (view === 'leaderboard' || session.status === 'finished') return (
    <div style={{ padding: '24px 16px 80px' }}>
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        <div style={{ fontSize: 56, marginBottom: 8 }}>{session.status === 'finished' ? '🏆' : '📊'}</div>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        <Leaderboard session={session} highlight={member?.id} showReview={showReview} onToggleReview={() => setShowReview(v => !v)} />
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 20 }}>
          ...
          {session.status === 'finished' && (
            <div style={{ borderRadius: 16, padding: 20, background: 'rgba(255,255,255,0.03)', border: '1.5px solid rgba(255,255,255,0.08)', textAlign: 'center' }}>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 600, marginBottom: 12 }}>Game over! Ready for another round?</div>
              <div style={{ fontSize: 12, color: 'var(--primary)', fontWeight: 700, marginBottom: 14 }}>Next up: {CATEGORY_NAMES[(sessionCount + 1) % 6]}</div>
              {isParent ? (
                <button onClick={handleNewGame} style={{ width: '100%', padding: '14px', borderRadius: 12, fontSize: 15, fontWeight: 900, background: 'linear-gradient(135deg,#6366F1,#A78BFA)', border: 'none', color: '#fff', cursor: 'pointer', boxShadow: '0 4px 16px rgba(99,102,241,0.4)' }}>Start New Game</button>
              ) : (
                <div style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 600 }}>Ask a parent to start a new game!</div>
              )}
            </div>
          )}
        </div>
  ...
  return (
    <div style={{ padding: '16px 16px 80px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 64, height: 64 }}>
            <svg width='64' height='64' style={{ position: 'absolute', transform: 'rotate(-90deg)' }}>
              ...
              <circle cx='32' cy='32' r='28' fill='none' stroke={timeLeft <= 5 ? '#F87171' : timeLeft <= 10 ? '#f59e0b' : '#6366f1'} strokeWidth='4' strokeDasharray={2 * Math.PI * 28} strokeDashoffset={2 * Math.PI * 28 * (1 - timerPct / 100)} strokeLinecap='round' style={{ transition: 'stroke-dashoffset 1s linear, stroke 0.3s' }} />
            </svg>
      ...
      {allDone && (
        <button onClick={handleFinish} disabled={finishing} style={{ width: '100%', padding: '16px', borderRadius: 16, fontSize: 16, fontWeight: 900, background: finishing ? 'rgba(74,222,128,0.1)' : 'linear-gradient(135deg, #4ADE80, #22C55E)', border: 'none', color: finishing ? '#4ADE80' : '#0f0f13', cursor: finishing ? 'not-allowed' : 'pointer', boxShadow: finishing ? 'none' : '0 8px 24px rgba(74,222,128,0.35)', marginBottom: 12 }}>
          {finishing ? 'Finishing...' : 'Done! See Final Results'}
        </button>
      )}
      {isParent && !allDone && (
